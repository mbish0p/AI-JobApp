...
class EmployeerPicModel extends React.Component {
    constructor(props) {
        super(props)
    ...
    uploadImage = async (image) => {
        ...
        data.append('logo', image, 'user_pic.png');

        try {
            const uploadImageResponse = await axios.post('http://localhost:5000/employeer/company-logo', data, {
                headers: {
            ...
            if (error.response.data.error.message === 'jwt expired')
                try {
                    ...
                    if (result.status === 201) {
                        const finallResult = await axios.post('http://localhost:5000/employeer/company-logo', data, {
                            headers: {
                                'Content-Type': `multipart/form-data; boundary=${data._boundary}`
                            }, withCredentials: true
                        })
...
export default EmployeerPicModel